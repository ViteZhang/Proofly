...
test("块的时间 · 挂了履历就写这条经历自己的起止，模型写的不算", () => {
  // 完整任职起止落在雇主标题行上（见 layout.ts），项目行说项目自己的时间。
  // 模型写的 2023.05 与经历自己的 2023.05 一致时看不出差别，所以这里让
  // 模型写一个别的值，确认它没被采信。
  assert.equal(blockMeta(true, "2019.01 – 至今", "2023.05 – 至今"), "2023.05 – 至今");
});

test("块的时间 · 挂了履历但经历自己没时间，才退回模型写的", () => {
  assert.equal(blockMeta(true, "2023.05 – 至今", ""), "2023.05 – 至今");
});

test("块的时间 · 没挂履历时用模型写的", () => {
  assert.equal(blockMeta(false, "2021.06 – 2023.02", "兜底"), "2021.06 – 2023.02");
});

test("块的时间 · 模型没写就用经历自己的时间兜底", () => {
  assert.equal(blockMeta(false, "   ", "2021.06 – 2023.02"), "2021.06 – 2023.02");
});
