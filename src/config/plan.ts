 ...
 * 发码的成本折算系数：1 分 ≈ ¥0.093。
 *
 * 来自商业化方案 9.3 的篮子实测 —— 完整求职包 200 分对应约 ¥18.57 的
 * 模型成本。**这是估算，不是精确成本**，9.4 已标注样本量不足，实际
 * 消耗取决于用户跑哪些动作。
 *
 * 它的作用不是算账，是在你按下「生成」之前把「发码就是发钱」变成一个
 * 有痛感的数字。积分这个单位没有痛感，¥ 有 —— 即使是粗估，它也挡得住
 * 「手滑发了 500 张 500 分」那一类事故（那是 ¥23,250）。
 */
export const CREDIT_COST_CNY = 0.093;
